...
export function encodeParameters(types: (string | utils.ParamType)[], values: any[]) {
  const abi = new ethers.utils.AbiCoder()
  return abi.encode(types, values)
}

export async function prepare(thisObject: any, contracts: string[]) {
...
export async function deploy(thisObject: any, contracts: ([string, ContractFactory] | [string, ContractFactory, any[]])[]) {
  for (let i in contracts) {
    let contract = contracts[i]
...
export async function createSLP(thisObject: any, name: string, tokenA: Contract, tokenB: Contract, amount: any) {
